...
* 람다함수
	* lambda는 람다 표현식, 람다함수, 익명함수 등의 이름으로 불린다
	* [] (매개변수) {// 함수동작} (호출 시 인자) ;  이런 식으로 람다 함수가 이루어져 있다.
		* 첫 번째 [] : 캡처 
		* 두 번째 () : 매개변수 선언 부분 (생략 가능 - 매개변수 필요 없을때)
		* 세 번째 {} : 함수 동작 부분
		* 네 번째 () : 함수 호출 시 인자 (생략 가능 - 호출 시에만 사용)
* 함수포인터
	* 함수의 주소를 저장하는 변수
	* 반환형(*변수이름)(매개변수형); 이런식으로 선언할 수 있다.
* thiscall
	* 함수호출 규약중 하나
	* 클래스의 멤버함수에 대해서만 적용되는데, ecx로 객체의 포인터(this)가 전달 된다는 것이다.
	* 컴파일러가 멤버 함수에 대해서만 특별히 적용하는 것
...
* 여러개의 프로세서가 작업을 병렬처리 하는 것
* 여러개의 단일 프로세싱보다 비용이 절약될 수 있다.
* 한 프로세서가 고장나더라도 시스템이 정지하는 것이 아니라 단지 속도만 느려지게 된다.
* 대칭형 멀티 프로세싱(SMP, Symmetric Multi-processing)
	* 운영체계와 메모리를 공유하는 여러 프로세서가 프로그램을 수행하는 것을 말한다.
	* 프로세서가 메모리와 임출력 버스 및 데이터 path를 공유하며, 하나의 운영체계가 모든 프로세서를 관리한다. 
	* 자원들을 서로 공유하면서 동기화가 필요하다.
* 비대칭형 멀티 프로세싱(AMP, Asymmetric Multi-processing)
	* 각 프로세서가 특정된 일을 맡아서 한다. 주 프로세서가 전체 시스템을 통제하고 다른 프로세서들은 주인의 명령을 따르거나 또는 미리 정해진 일을 수행한다.
	* 주-종속(master-slave) 시스템의 형태로서 주 프로세서가 종속프로세서들을 스케쥴하고 일을 할당한다.

### 멀티스레딩(Multi-threading)
* 하나의 프로세스 내에서 둘 이상의 스레드가 동시에 작업을 수행하는 것
* 각 스레드가 자신이 속한 프로세스의 메모리를 공유한다.
* 프로세서가 여러개인 경우 멀티 스레드를 통해 병렬성을 높일 수 있다.  이는 프로세스의 스레드들이 각각 다른 스로세서에서 병력적으로 수행될 수 있기 때문이다.
* 프로세서가 하나인 경우엔 멀티스레드를 통해서 동시성을 높일 수 있다. 실제로는 각각의 시간에 한 작업만 수행되지만, 병렬적으로 수행되는 것처럼 보이는 것이다. 
* 장점
	* 응답성 : 작업을 분리해서 수행하므로 실시간으로 사용자에게 응답할 수 있다.
	* 자원 공유 : 자신이 속한 프로세스 내의 스레드들과 메모리나 자원을 공휴
	* 경제성 : 프로세스를 새로 생성하는 비용보다 스레드를 새로 생성하는게 훨씬 싸다.  
	* 확장성 : 싱글 스레드인 경우 한 프로세스는 오직 한 프로세서에서만 수행 가능하다. 반면 멀티 스레드인 경우 한 프로세스를 여러 프로세서에서 수행할 수 있으름로 훨씬 효율적이다.

### 동기, 비동기
* 동기(synchronous) : 요청과 그 결과가 동시에 일어난다는 약속
* 비동기(asynchronous) : 요청과 결과가 동시에 일어나지 않을거라는 약속
* 동기방식은 설계가 매우 간단하고 직관적이지만 결과가 주어질 때까지 아무것도 못하고 대기해야 하는 단점이 있다.
* 비동기 방식은 동기보다 복잡하지만 결과가 주어지는데 시간이 걸리더라도 그 시간동안 다른 작업을 할 수 있으므로 자원을 효율적로 사용할 수 있는 장점이 있습니다.

### 임계 구역 (critical section)
* 공유변수 영역이라고도 한다.
* 동시에 접근해서는 안되는 구역으로서 동기화가 필요하다.

### 교착상태 (deadlock)
* 두 개 이상의 작업이 서로 상대방의 작업이 끝나기 만을 기다리고 있기 때문에 결과적으로 아무것도 완료되지 못하는 상태를 가리킨다.
* 교착 상태의 조건
	1. 상호배제 : 프로세스들이 필요로 하는 자원에 대해 배태적인 통제권을 요구한다.
	2. 점유대기 : 프로세스가 할당된 자원을 가진 상태에서 다른 자원을 기다린다.
	3. 비선점 : 프로세스가 어떤 자원의 사용을 끝낼 떄까지 그 자원을 뺏을 수 없다.
...
### 알파
* 알파 테스트
	* 알파테스트는 조각의 알파 값과 상수 참조 값 간의 비교 결과에 따라 조각을 삭제합니다. 
	* 반투명 이란것은 없고 이 픽셀이 보이는가 안보이는 가만 임의대로 결정해 버리는 것. (0 or 1)
* 알파 블렌딩
	* 모든 픽셀은 이 픽셀이 보인다는 가정하에 얼마나 보이는가를 계산해야 한다.
	* 중간에 불투명한 오브젝트가 끼어있을지도 모르기 때문에 깊이 테스트가 필요하다.
	* 알파채널 값을 이용해 색을 섞는다.
* 알파 소팅
	* 알파가 있는것들과 알파가 없는것들을 따로 그리는 것.
	* 알파가 없는 오브젝트들은 화면 앞의 오브젝트부터 뒤에있는 오브젝트 순으로 렌더링하는게 빠르다.
	* 하지만 반투명, 투명 오브젝트는 뒤에서부터 그려야 한다. 
	* 그래서 두 오브젝트를 따로 렌더링하는 것이 효율적
* Z-fighting
	* 2개의 매쉬의 깊이가 동일하거나 유사하여 무엇이 우선으로 렌더링 되어야할지 모르는 상황이 발생하게 되면 두 매쉬가 겹쳐보이는 현상
	* 해결법
		1. 렌더링의 우선순위 값을 조정한다.
		2. 매쉬 사이의 거리를 벌린다.
